...
@torch.no_grad()
def count_ops_and_params(model, example_inputs, inplace=False, layer_wise=False, block_wise=False, branch_wise=False, custom_layer_name=[]):
    global CUSTOM_MODULES_MAPPING
    ori_model = model 
    # model = copy.deepcopy(model).cpu() # deepcopy to avoid changing the original model; move device to cpu to avoid cuda memory usage
    if inplace:
        model = model.cpu()
    else:
    ...
    block_flops = {
        "mid_block": 0,
        "up_blocks.0": 0,  "up_blocks.1": 0, "up_blocks.2": 0, "up_blocks.3": 0,
        "down_blocks.0": 0,  "down_blocks.1": 0, "down_blocks.2": 0, "down_blocks.3": 0,
    }
    block_params = {
        "mid_block": 0,
        "up_blocks.0": 0,  "up_blocks.1": 0, "up_blocks.2": 0, "up_blocks.3": 0,
    ...
    for m_name, m in model.named_modules():
        if m_name in layer_flops.keys(): continue
        layer_flops[m_name] = _layer_flops.get(m)
        layer_params[m_name] = _layer_params.get(m)

        if m_name in block_flops.keys():
            block_flops[m_name] = _layer_flops.get(m)
            block_params[m_name] = _layer_params.get(m)
    ...
    # get branch-wise flops
    sub_block_flops = {}
    branch_i = 1
    for block_i, block in enumerate(model.down_blocks):
        for (layer_i, _) in enumerate(getattr(block, "attentions", [])):
            layer_name = f"down_blocks.{block_i}.attentions.{layer_i}"
            block_name = ("down", "attentions", block_i, layer_i, branch_i + layer_i)
            sub_block_flops[block_name] = layer_flops[layer_name]

        for (layer_i, _) in enumerate(getattr(block, "resnets", [])):
            layer_name = f"down_blocks.{block_i}.resnets.{layer_i}"
            block_name = ("down", "resnet", block_i, layer_i, branch_i + layer_i)
            sub_block_flops[block_name] = layer_flops[layer_name]

        branch_i += len(getattr(block, "resnets", []))
        for _ in getattr(block, "downsamplers", []) if block.downsamplers else []:
            layer_name = f"down_blocks.{block_i}.downsamplers"
            block_name = ("down", "downsampler", block_i, len(getattr(block, "resnets", [])), branch_i)
            sub_block_flops[block_name] = layer_flops[layer_name]
            branch_i += 1

    sub_block_flops[("mid", "mid_block", 0, 0, branch_i)] = layer_flops["mid_block"]

    branch_i = branch_i - 1
    block_num = len(model.up_blocks)
    for block_i, block in enumerate(model.up_blocks):
        layer_num = len(getattr(block, "resnets", []))      # 3
        for (layer_i, _) in enumerate(getattr(block, "attentions", [])):
            layer_name = f"up_blocks.{block_i}.attentions.{layer_i}"
            block_name = ("up", "attentions", block_num - block_i - 1, layer_num - layer_i - 1, branch_i - layer_i)
            sub_block_flops[block_name] = layer_flops[layer_name]
        for (layer_i, _) in enumerate(getattr(block, "resnets", [])):
            layer_name = f"up_blocks.{block_i}.resnets.{layer_i}"
            block_name = ("up", "resnet", block_num - block_i - 1, layer_num - layer_i - 1, branch_i - layer_i)
            sub_block_flops[block_name] = layer_flops[layer_name]
        for _ in getattr(block, "upsamplers", []) if block.upsamplers else []:
            layer_name = f"up_blocks.{block_i}.upsamplers"
            block_name = ("up", "upsampler", block_num - block_i - 1, 0, branch_i - layer_num + 1)
            sub_block_flops[block_name] = layer_flops[layer_name]
        branch_i = branch_i - layer_num

    sub_block_flops_ratios = {k: v / flops_count for k, v in sub_block_flops.items()}

    branch_flops = {}
    branch_flops_ratios = {}
    for k, v in sub_block_flops_ratios.items():
        blocktype, _, _, _, branch_i = k
        if (blocktype, branch_i) not in branch_flops.keys():
            branch_flops[(blocktype, branch_i)] = sub_block_flops[k]
            branch_flops_ratios[(blocktype, branch_i)] = v
        else:
            branch_flops[(blocktype, branch_i)] += sub_block_flops[k]
            branch_flops_ratios[(blocktype, branch_i)] += v
    if branch_wise:
        for k in branch_flops.keys():
            blocktype, branch_i = k
            space = 30 - len(blocktype)
            print("Block {} branch {}: {} MACs = {:.4f} G, MACs% = {:.2f}".format(
                blocktype, branch_i, ' ' * space, branch_flops[k] / 1e9, 100 * branch_flops_ratios[k]
            ))
    # print(branch_flops.keys())
    # block-wise
    block_flops_ratios = {k: v / flops_count for k, v in block_flops.items()}
    for block_name in block_flops.keys():
        space = 30 - len(block_name)
        if block_wise:
            print("Block {}: {} MACs = {:.4f} G, Params = {:.4f} M, MACs% = {:.2f}".format(
                block_name, ' ' * space, block_flops[block_name] / 1e9, block_params[block_name] / 1e6,
                              100 * block_flops[block_name] / flops_count
            ))

    # total_sub_blocks_flops_ratio = sum(list(sub_block_flops_ratios.values()))
    # total_blocks_flops_ratio = sum(list(block_flops_ratios.values()))
    # print(total_blocks_flops_ratio, total_sub_blocks_flops_ratio)
    # assert total_sub_blocks_flops_ratio == total_blocks_flops_ratio

    flops_model.stop_flops_count()
    CUSTOM_MODULES_MAPPING = {}

    # block_flops["total_macs"] = flops_count
    #if layer_wise:
    #    return flops_count, params_count, layer_flops, layer_params
    del model, flops_model
    torch.cuda.empty_cache()

    return flops_count, params_count, block_flops, block_params, block_flops_ratios, sub_block_flops, sub_block_flops_ratios, branch_flops, branch_flops_ratios
