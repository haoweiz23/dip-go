...
def get_pil_activations(files, model, batch_size=50, dims=2048, device='cpu',
                    num_workers=1, res=None, dataset_name=None):
    """Calculates the activations of the pool_3 layer for all images.
    ...
    dataset = PILDataset(files, transforms=trans)
    dataloader = torch.utils.data.DataLoader(dataset,
                                             batch_size=batch_size,
                                             shuffle=False,
                                             drop_last=False,
...
def calculate_activation_statistics(files, model, batch_size=50, dims=2048,
                                    device='cpu', num_workers=1, res=None, dataset_name=None, type=None):
    ...
    """
    if type == "pil":
        act = get_pil_activations(files, model, batch_size, dims, device, num_workers, res=res, dataset_name=dataset_name)
    else:
...
def compute_statistics_of_path(path, model, batch_size, dims, device,
    ...
    if path.endswith('.pt'):
        ...
        files = [i for i in ckpt['images']]
        # files = ckpt['images']
        # print(files.shape)
        # exit()
        from torchvision.transforms.functional import to_pil_image
        files = [to_pil_image(i, mode=None) for i in files]
        if num_samples is not None:
            files = files[:num_samples]
        print("Found %d files." % len(files))
        m, s = calculate_activation_statistics(files, model, batch_size,
                                               dims, device, num_workers, type='pil', res=res, dataset_name=dataset_name)
