...
from .pruner import SDXL_BRANCH_NAME
from types import MethodType
from .unet_2d_condition import pruner_forward, UNet2DConditionModel
...
class StableDiffusionXLPipeline(
    ...
    def __init__(
        ...
        unet = UNet2DConditionModel.from_config(self.unet.config)
        unet.load_state_dict(self.unet.state_dict())
        self.unet = unet
        # self.unet = UNet2DConditionModel.from_config(self.unet.config)
        # self.unet.forward = MethodType(pruner_forward, self.unet)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if gates is not None:
            all_timestep_gates = [
                {k: v.to(device) for k, v in gate.items()} for gate in gates
            ]
        else:
            all_timestep_gates = [
                {k: torch.Tensor([0, 1]).to(device) for k in SDXL_BRANCH_NAME}
                for _ in range(num_inference_steps)
            ]

            # all_timestep_gates[2][('mid', 9)] = torch.Tensor([1, 0]).to(device)
            # all_timestep_gates[3][('mid', 9)] = torch.Tensor([1, 0]).to(device)
            # print(all_timestep_gates)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i == 0:
                    prv_features = None
                is_gates_train = False  # TODO: debug mode
                ...
                noise_pred, prv_features = self.unet(
                    ...
                    gates=all_timestep_gates[i - 1] if i > 0 else None,
                )
                # -----
                all_xts.append(prv_features[-2].mean((2, 3)).chunk(2)[1])
                # perform guidance
                if self.do_classifier_free_guidance:
    ...
    def pruner_forward(
        ...
        ip_adapter_image: Optional[PipelineImageInput] = None,
        ip_adapter_image_embeds: Optional[List[torch.FloatTensor]] = None,
        output_type: Optional[str] = "vae_decode",
        return_dict: bool = True,
        cross_attention_kwargs: Optional[Dict[str, Any]] = None,
        guidance_rescale: float = 0.0,
        original_size: Optional[Tuple[int, int]] = None,
        ...
        **kwargs,
        ...
        if gates is not None:
            ...
            # all_timestep_gates[0][('down', 5)] = torch.Tensor([1, 0]).to(device)
            # all_timestep_gates[0][('up', 5)] = torch.Tensor([1, 0]).to(device)
            # all_timestep_gates[2][('down', 5)] = torch.Tensor([1, 0]).to(device)
            # all_timestep_gates[2][('up', 5)] = torch.Tensor([1, 0]).to(device)
            # all_timestep_gates[2][('mid', 9)] = torch.Tensor([1, 0]).to(device)
            # all_timestep_gates[3][('mid', 9)] = torch.Tensor([1, 0]).to(device)
            # print(all_timestep_gates)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred, prv_features = self.unet(
                    ...
                    added_cond_kwargs=added_cond_kwargs,
                    return_dict=False,
                    is_gates_train=is_gates_train if i > 0 else False,
                    replicate_prv_feature=prv_features,
                    gates=all_timestep_gates[i - 1] if i > 0 else None,
                )
                # -----
    ...
    # TODO: sdxl is not use vae output for training, cause it need to set vae
    # is in float32 mode and problems rise in gradient backward.
