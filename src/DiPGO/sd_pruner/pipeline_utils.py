...
class DiffusionPipeline(ConfigMixin, PushToHubMixin):
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        model_index_dict = {
            k: v for k,
            v in model_index_dict.items() if is_saveable_module(
                k,
                v)}
        for pipeline_component_name in model_index_dict.keys():
            ...
            for library_name, library_classes in LOADABLE_CLASSES.items():
                ...
                for base_class, save_load_methods in library_classes.items():
                    ...
                    if class_candidate is not None and issubclass(
                        ...
                        # if we found a suitable base class in LOADABLE_CLASSES
                        # then grab its save method
            ...
            if save_method_name is None:
                ...
                # make sure that unsaveable components are not tried to be
                # loaded afterward
            ...
            # Call the save method with the argument safe_serialization only if
            # it's supported
    ...
    def to(self, *args, **kwargs):
        ...
        if len(args) == 1:
            if isinstance(args[0], torch.dtype):
                ...
                device_arg = torch.device(
                    args[0]) if args[0] is not None else None
        elif len(args) == 2:
            if isinstance(args[0], torch.dtype):
                raise ValueError(
        ...
        # throw warning if pipeline is in "offloaded"-mode but user tries to
        # manually set to GPU.
        ...
        def module_is_offloaded(module):
            ...
            return hasattr(module, "_hf_hook") and isinstance(
                module._hf_hook, accelerate.hooks.CpuOffload
            )

        # .to("cuda") would raise an error if the pipeline is sequentially offloaded, so we raise our own to make it clearer
        pipeline_is_sequentially_offloaded = any(
            module_is_sequentially_offloaded(module)
            for _, module in self.components.items()
        )
        if (
        ...
        # Display a warning in this case (the operation succeeds but the
        # benefits are lost)
        pipeline_is_offloaded = any(
            module_is_offloaded(module) for _,
            module in self.components.items())
        if pipeline_is_offloaded and device and torch.device(
                device).type == "cuda":
            logger.warning(
        ...
        for module in modules:
            is_loaded_in_8bit = (
                hasattr(
                    module,
                    "is_loaded_in_8bit") and module.is_loaded_in_8bit)
    ...
    def from_pretrained(
        ...
        if variant is not None:
            for folder in os.listdir(cached_folder):
                ...
                is_folder = os.path.isdir(
                    folder_path) and folder in config_dict
        ...
        if os.path.isfile(
            os.path.join(
                cached_folder,
                f"{custom_pipeline}.py")):
            custom_pipeline = os.path.join(
                cached_folder, f"{custom_pipeline}.py")
            ...
            custom_pipeline = os.path.join(
                cached_folder, f"{config_dict['_class_name'][0]}.py"
            )
        ...
        passed_pipe_kwargs = {k: kwargs.pop(k)
        ...
        # define init kwargs and make sure that optional component modules are
        # filtered out
        init_kwargs = {
            k: init_dict.pop(k)
        ...
        # Special case: safety_checker must be loaded separately when using
        # `from_flax`
        if (
            from_flax
        ...
        for name, (library_name, class_name) in logging.tqdm(
            ...
            # 6.1 - now that JAX/Flax is an official framework of the library,
            # we might load from Flax names
            class_name = class_name[4:] if class_name.startswith(
                "Flax") else class_name
        ...
        if pipeline_class._load_connected_pipes and os.path.isfile(
            ...
            for prefix, connected_pipe in connected_pipes.items():
                # add connected pipes to `init_kwargs` with
                # <prefix>_<component_name>, e.g. "prior_text_encoder"
                init_kwargs.update(
                    {
    ...
    def enable_model_cpu_offload(self,
        ...
        # _offload_gpu_id should be set to passed gpu_id (or id in passed
        # `device`) or default to previously set id or default to 0
        ...
        if self.device.type != "cpu":
            ...
            if hasattr(
                    device_mod,
                    "empty_cache") and device_mod.is_available():
                # otherwise we don't see the memory savings (but they probably
                # exist)
                device_mod.empty_cache()

        all_model_components = {
            k: v for k, v in self.components.items() if isinstance(
                v, torch.nn.Module)}
        ...
        for model_str in self.model_cpu_offload_seq.split("->"):
            ...
            _, hook = cpu_offload_with_hook(
                model, device, prev_module_hook=hook)
            self._all_hooks.append(hook)

        # CPU offload models that are not in the seq chain unless they are explicitly excluded
        # these models will stay on CPU until maybe_free_model_hooks is called
        # some models cannot be in the seq chain because they are iteratively
        # called, such as controlnet
    ...
    def download(cls, pretrained_model_name, **
        ...
        if not local_files_only:
            ...
            folder_names = [
                ...
                if isinstance(v, list) and k != "_class_name"
            ]
            ...
            # pipelines = getattr(diffusers_module, "sd_pruner")
            ...
            for component in folder_names:
                ...
                if module_candidate is None or not isinstance(
                        module_candidate, str):
                    continue

                # We compute candidate file path on the Hub. Do not use
                # `os.path.join`.
            ...
            if revision in DEPRECATED_REVISION_ARGS and version.parse(
                ...
                warn_deprecated_model_variant(
                    pretrained_model_name, token, variant, revision, model_filenames)

            model_folder_names = {
                os.path.split(f)[0]
                for f in model_filenames
                if os.path.split(f)[0] in folder_names
            }

            custom_class_name = None
            if custom_pipeline is None and isinstance(
                config_dict["_class_name"], (list, tuple)
            ):
            ...
            allow_patterns += [
                f"{k}/*" for k in folder_names if k not in model_folder_names
            ]
            # add custom component files
            allow_patterns += [f"{k}/{f}.py" for k,
                               f in custom_components.items()]
            # add custom pipeline file
            allow_patterns += (
                [f"{custom_pipeline}.py"]
                if f"{custom_pipeline}.py" in filenames
                else []
            )
            ...
            load_pipe_from_hub = (
                custom_pipeline is not None and f"{custom_pipeline}.py" in filenames)
            ...
            if from_flax:
                ...
                safetensors_model_filenames = {
                    f for f in model_filenames if f.endswith(".safetensors")
                }
                if (
                ...
                bin_variant_filenames = {
                    f for f in variant_filenames if f.endswith(".bin")
                }
                bin_model_filenames = {
                    f for f in model_filenames if f.endswith(".bin")}
                if (
                    len(bin_variant_filenames) > 0
                    and bin_model_filenames != bin_variant_filenames
                ):
                    logger.warning(
            ...
            expected_files = [
                f for f in filenames if not any(
                    p.match(f) for p in re_ignore_pattern)]
            expected_files = [
                f for f in expected_files if any(
                    p.match(f) for p in re_allow_pattern)]
        ...
        try:
            ...
            cls_name = cls.load_config(
                os.path.join(cached_folder, "model_index.json")
            ).get("_class_name", None)
            cls_name = (
                cls_name[4:]
                if isinstance(cls_name, str) and cls_name.startswith("Flax")
                else cls_name
            )

            diffusers_module = importlib.import_module(__name__.split(".")[0])
            pipeline_class = (
                getattr(diffusers_module, cls_name, None)
            ...
            if pipeline_class is not None and pipeline_class._load_connected_pipes:
                modelcard = ModelCard.load(
                    os.path.join(cached_folder, "README.md"))
                connected_pipes = sum(
                    [getattr(modelcard.data, k, []) for k in CONNECTED_PIPES_KEYS], []
                )
                for connected_pipe_repo_id in connected_pipes:
                    ...
                    DiffusionPipeline.download(
                        connected_pipe_repo_id, **download_kwargs
                    )
            ...
            # 2. If we forced `local_files_only=True` when `model_info` failed
            # => we raise the initial error
    ...
    def _get_signature_keys(cls, obj):
        ...
        optional_parameters = set(
            {k for k, v in parameters.items() if v.default != inspect._empty}
        )
    ...
    def set_attention_slice(self, slice_size: Optional[int]):
        ...
        modules = [getattr(self, n, None) for n in module_names]
        modules = [
            m for m in modules if isinstance(
                m, torch.nn.Module) and hasattr(
                m, "set_attention_slice")]
