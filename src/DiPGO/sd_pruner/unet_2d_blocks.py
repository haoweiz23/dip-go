    ...
    def forward(
        ...
        if is_gates_train:
            ...
            hidden_states = hidden_states * \
                gates[1] + replicate_prv_feature * gates[0]
        else:
            if is_reuse(gates):
                hidden_states = replicate_prv_feature
            else:
                hidden_states = self.resnets[0](hidden_states, temb)
                for attn, resnet in zip(self.attentions, self.resnets[1:]):
                    if self.training and self.gradient_checkpointing:
        ...
        prv_features = [hidden_states]
        return hidden_states, prv_features
...
class UNetMidBlock2DSimpleCrossAttn(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is None:
            # if encoder_hidden_states is defined: we are doing cross-attn, so
            # we should use cross-attn mask.
    ...
    def forward(
        ...
        for i, (resnet, attn) in enumerate(blocks):
            if is_gates_train:
                ...
                # apply additional residuals to the output of the last pair of
                ...
                hidden_states = (
                    hidden_states *
                    gates[i][1] +
                    replicate_prv_feature[i] *
                    gates[i][0])

            else:
                if is_reuse(gates[i]):
                    # 1. replicate previous feature into current status
                    # 2. else save the previous feature into prv_features
                    hidden_states = replicate_prv_feature[i]
                else:
                    if self.training and self.gradient_checkpointing:
                    ...
                    # apply additional residuals to the output of the last pair
                    # of resnet and attention blocks
                    if i == len(blocks) - \
                            1 and additional_residuals is not None:
                        hidden_states = hidden_states + additional_residuals
        ...
        if self.downsamplers is not None:
            if is_gates_train:
                ...
                hidden_states = (hidden_states *
                                 gates[len(blocks)][1] +
                                 replicate_prv_feature[len(blocks)] *
                                 gates[len(blocks)][0])
            else:
                if is_reuse(gates[len(blocks)]):
                    hidden_states = replicate_prv_feature[len(blocks)]
                else:
                    for downsampler in self.downsamplers:
                        hidden_states = downsampler(hidden_states)
        ...
        prv_features = list(output_states)
        return hidden_states, output_states, prv_features
        ...
        if self.downsamplers is not None:
            if is_gates_train:
                for downsampler in self.downsamplers:
                    hidden_states = downsampler(hidden_states)
                hidden_states = (
                    hidden_states * gates[len(self.resnets)][1]
                    + replicate_prv_feature[len(self.resnets)]
                    * gates[len(self.resnets)][0]
                )
            else:
                if is_reuse(gates[len(self.resnets)]):
                    hidden_states = replicate_prv_feature[len(self.resnets)]
                else:
                    for downsampler in self.downsamplers:
                        hidden_states = downsampler(hidden_states)
...
class KDownBlock2D(nn.Module):
    def __init__(
        ...
        if add_downsample:
            # YiYi's comments- might be able to use FirDownsample2D, look into
            # details later
    ...
    def forward(
        ...
        for i, (resnet, attn) in enumerate(zip(self.resnets, self.attentions)):
            ...
            if is_gates_train:
                ...
                if i < len(self.resnets) - 1 or (
                    i == len(self.resnets) - 1 and self.upsamplers is None
                ):
                    hidden_states = (
                        hidden_states * gates[i][1]
                        + replicate_prv_feature[i] * gates[i][0]
                    )
            else:
                if is_reuse(gates[i]):
                    if i < len(self.resnets) - 1 or (
            ...
            if i < len(self.resnets) - 1 or (
                ...
                prev_features = prev_features + (hidden_states,)
        ...
        if self.upsamplers is not None:
            if is_gates_train:
                ...
                hidden_states = (
                    hidden_states * gates[len(self.resnets) - 1][1]
                    + replicate_prv_feature[len(self.resnets) - 1]
                    * gates[len(self.resnets) - 1][0]
                )
            else:
                if is_reuse(gates[len(self.resnets) - 1]):
                    hidden_states = replicate_prv_feature[len(
                        self.resnets) - 1]
                else:
                    for upsampler in self.upsamplers:
                        hidden_states = upsampler(hidden_states, upsample_size)
            ...
            prev_features = prev_features + (hidden_states,)
        prev_features = list(prev_features)
        return hidden_states, prev_features
        ...
        for i in range(num_layers):
            ...
            self.resnets.append(
                ResnetBlock2D(
                    ...
                    eps=resnet_eps,
                    groups=min(
                        (resnet_in_channels + res_skip_channels) // 4,
                        32),
                    groups_out=min(
                        out_channels // 4,
                        32),
